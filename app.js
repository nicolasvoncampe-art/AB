...
function updateDashboard() {
    ...
    workoutRequest.onsuccess = () => {
        ...
        workouts.forEach(workout => {
            ...
            } else if (workout.type === 'situps') {
                totalSitups += parseInt(workout.value) || 0;
            } else if (workout.type === 'squats') {
                totalSquats += parseInt(workout.value) || 0;
            }
        });
        
        // Update home screen
        document.getElementById('homeRunningKm').textContent = totalKm.toFixed(1);
        document.getElementById('homePushups').textContent = totalPushups;
        
        // Update workout screen
        document.getElementById('workoutRunningKm').textContent = totalKm.toFixed(1);
        document.getElementById('workoutPushups').textContent = totalPushups;
        document.getElementById('workoutSitups').textContent = totalSitups;
        document.getElementById('workoutSquats').textContent = totalSquats;
    };
    ...
    foodRequest.onsuccess = () => {
        ...
        // Update home screen
        document.getElementById('homeCalories').textContent = totalCalories;
        document.getElementById('homeProtein').textContent = totalProtein;
        
        // Update food screen
        document.getElementById('foodCalories').textContent = totalCalories;
        document.getElementById('foodProtein').textContent = totalProtein;
    };
...
function renderCalendar() {
    ...
    getMonthActivities(year, month).then(activities => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (activities[dateStr]) {
                ...
                const activityCount = Math.min(activities[dateStr], 3);
...
function getMonthActivities(year, month) {
    return new Promise((resolve) => {
        ...
        Promise.all([
            new Promise(res => { workoutRequest.onsuccess = () => res(workoutRequest.result); }),
            new Promise(res => { foodRequest.onsuccess = () => res(foodRequest.result); })
        ]).then(([workouts, foods]) => {
            [...workouts, ...foods].forEach(entry => {
                const entryDate = new Date(entry.date);
                if (entryDate.getFullYear() === year && entryDate.getMonth() === month) {
                    activities[entry.date] = (activities[entry.date] || 0) + 1;
                }
            });
            resolve(activities);
        });
...
function saveFood() {
    ...
    const calories = document.getElementById('foodCaloriesInput').value;
